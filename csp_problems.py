...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for i, row in enumerate(var_array):
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint('row' + str(i), row))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint('square' + str(i) + str(j), scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for p in planes_problem.planes:
        ...
        for i in range(0, len(flight_sequence) - 1):
            ...
            constraints.append(TableConstraint(p + '_order%d' % i, [f1, f2], legal_order))

        # Flight maintenance
        if len(flight_sequence) >= planes_problem.min_maintenance_frequency:
            for i in range(0, len(flight_sequence) - planes_problem.min_maintenance_frequency + 1):
                fs = flight_sequence[i:i + planes_problem.min_maintenance_frequency]
                constraints.append(NValuesConstraint(p + '_maintenance%d' % i, fs, maintenance_flights,
                                                     1, len(flight_sequence)))
