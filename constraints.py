...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        c = []
        for vali in range(1, qi.domainSize() + 1):
            for valj in range(1, qj.domainSize() + 1):
                if vali != valj and abs(vali - valj) != abs(i - j):
                    c.append([vali, valj])
        TableConstraint.__init__(self, 'QueenTableCnstr_' + 'name', [qi, qj], c)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def valsSatisfy(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the all diff'''
            vals = [val in self._required for (var, val) in l]
            return self._lb <= sum(vals) <= self._ub
        def partialValsSatisfy(l):
            vals = [val in self._required for (var, val) in l]
            s = sum(vals)
            if s > self._ub:
                return False
            remaining = len(self.scope()) - len(vals)
            if remaining + s < self._lb:
                return False
            return True

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsSatisfy, partialValsSatisfy)
        return x
